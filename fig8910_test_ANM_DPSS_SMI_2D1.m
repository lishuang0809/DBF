...
% This code is used to reproduce Figures 8-10.
...
% Signal setup
ns = 30; % number of samples % ns=30 for zigzag ns=15 for others
...
% === linear freq drift ===
% fs = 0.002*t; %0.0005*rand(ns,1);  %fs = 0.0001*t; % intended signal frequency offset
% fi = t*[-0.001 -0.003]; %rand(ns,1)*[0.0005 0.001];%fi = t*[0.0001 0.0002];%[1/100 1/200]; % interferer frequency offset
% 
% fs = fs - mean(fs);
% fi = fi - mean(fi);
...
% === zigzag freq drift ===
fs = [-0.005*(0:9)';-0.005*9+0.005*(0:9)';-0.005*(0:9)']; %0.0005*rand(ns,1);  %fs = 0.0001*t; % intended signal frequency offset

fi = [-0.004*(0:9)' 0.002*(0:9)';-0.004*9+0.004*(0:9)' 0.002*9-0.002*(0:9)';-0.004*(0:9)' 0.002*(0:9)']; %0.0005*rand(ns,1);  %fs = 0.0001*t; % intended signal frequency offset
